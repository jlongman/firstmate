...
config/crew-sandbox  optional claude-crewmate OS-confinement selector; LOCAL, gitignored; absent or "off" = no sandbox (default, byte-identical launch), "srt"/"on" = wrap the launch in Anthropic's sandbox-runtime or refuse if its preflight fails, "auto" = sandbox when the preflight passes else plain launch with a loud warning; claude ship/scout crewmates only, and fails closed elsewhere: strict "srt"/"on" REFUSES a non-claude crewmate or any secondmate while "auto" only warns and launches unconfined; not inherited into secondmate homes; see docs/configuration.md "Crew sandbox" and docs/crewmate-sandbox.md
